...
public class Almacen {
	...
	public Almacen(int fila, int columnas) {
	...
	public void agregarBebida(Bebida b) {
		...
		if(encontrado) {
			...
			System.out.println("No se a podido añadir la bebida");
		}
	}
		
		public void eleminarBebida(int id) {
		...
		public double calcularPrecioBebida(String marca) {
			double precioTotal=0;
			for(int i=0; i<estanteria.length; i++) {
				for(int j=0; j<estanteria[0].length; j++) {
					if(estanteria[i][j]!=null) {
		...
		public double calcularPrecioBebida(int columna) {
